#!/bin/bash
# ==========================================
# Check VLESS Users - FIXED VERSION
# ==========================================
...
# Function to get VLESS users using jq - FIXED
get_vless_users() {
    ...
    # Extract VLESS WS users - FIXED: handle empty clients array
    if jq -e '.inbounds[] | select(.tag == "vless-ws") | .settings.clients' "$config_file" > /dev/null 2>&1; then
        local ws_users=$(jq -r '.inbounds[] | select(.tag == "vless-ws") | .settings.clients[]? | .email // empty' "$config_file" 2>/dev/null)
        if [[ -n "$ws_users" ]]; then
    ...
    # Extract VLESS gRPC users - FIXED: handle empty clients array
    if jq -e '.inbounds[] | select(.tag == "vless-grpc") | .settings.clients' "$config_file" > /dev/null 2>&1; then
        local grpc_users=$(jq -r '.inbounds[] | select(.tag == "vless-grpc") | .settings.clients[]? | .email // empty' "$config_file" 2>/dev/null)
...
get_user_details() {
    ...
    # Get UUID from config - FIXED: handle empty results
    local uuid=$(jq -r '.inbounds[] | select(.tag == "vless-ws") | .settings.clients[]? | select(.email == "'"$user"'") | .id // empty' "$config_file" 2>/dev/null)
    
    # If not found in WS, check gRPC
    if [[ -z "$uuid" ]]; then
        uuid=$(jq -r '.inbounds[] | select(.tag == "vless-grpc") | .settings.clients[]? | select(.email == "'"$user"'") | .id // empty' "$config_file" 2>/dev/null)
    fi
...
get_user_services() {
    ...
    # Check VLESS WS
    if jq -e '.inbounds[] | select(.tag == "vless-ws") | .settings.clients[]? | select(.email == "'"$user"'")' "$config_file" &>/dev/null; then
        services="WS"
    fi
    
    # Check VLESS gRPC
    if jq -e '.inbounds[] | select(.tag == "vless-grpc") | .settings.clients[]? | select(.email == "'"$user"'")' "$config_file" &>/dev/null; then
        if [[ -n "$services" ]]; then
    ...
    for log_file in "${log_files[@]}"; do
        if [[ -f "$log_file" ]]; then
            ...
            for log in $recent_logs; do
                if [[ -f "$log" ]]; then
                    # Extract IPs with successful connections for this user/UUID
                    # Match various log formats
                    local user_ips=$(grep -E "($user|$uuid)" "$log" 2>/dev/null | \
...
get_user_creation_date() {
    local user="$1"
    local log_file="/var/log/create-vmess.log"
    
    if [[ -f "$log_file" ]]; then
        local created=$(grep -E "VMESS ACCOUNT CREATED.*$user" "$log_file" 2>/dev/null | \
...
# Main script
echo -e "${yellow}Loading VLESS users...${nc}"
...
if [[ ${#users[@]} -eq 0 ]] || [[ -z "${users[0]}" ]]; then
    ...
    echo -e "${red}=========================================${nc}"
    echo -e "${red}No VLESS users found in configuration${nc}"
    ...
    if [[ -f "/usr/local/etc/xray/config.json" ]]; then
        ...
        echo -e "${blue}VLESS WS clients count:${nc}"
        jq '[.inbounds[] | select(.tag == "vless-ws") | .settings.clients[]?] | length' /usr/local/etc/xray/config.json 2>/dev/null || echo "0"
        
        echo -e "${blue}VLESS gRPC clients count:${nc}"
        jq '[.inbounds[] | select(.tag == "vless-grpc") | .settings.clients[]?] | length' /usr/local/etc/xray/config.json 2>/dev/null || echo "0"
    else
...
echo -e "${green}Found ${#users[@]} VLESS user(s)${nc}"
echo ""

# Display all users first
echo -e "${blue}ALL VMESS USERS:${nc}"
echo -e "${red}=========================================${nc}"
...
for user in "${users[@]}"; do
    ...
    if [[ -n "$uuid" ]]; then
        active_ips=($(check_user_connections "$user" "$uuid"))
        
        if [[ ${#active_ips[@]} -gt 0 ]]; then
            ((active_users_count++))
            services=$(get_user_services "$user")
            expiry=$(get_user_details "$user" | cut -d'|' -f2)
            
            echo -e "${green}✓ $user${nc}"
            echo -e "  └─ Services: ${blue}$services${nc} | Expiry: ${yellow}$expiry${nc}"
            echo -e "  └─ UUID: ${yellow}${uuid:0:8}...${nc}"
