...
wget -q -O /usr/local/bin/proxy.js ${REPO_RAW}/ws-stunnel/proxy.js
...
wget -q -O /etc/systemd/system/ws-proxy.service ${REPO_RAW}/ws-stunnel/ws-proxy.service
...
echo -e "${red}=========================================${nc}"
# install tor openvpn
wget -q -O openvpn.sh "${REPO_RAW}/openvpn/openvpn.sh" && chmod +x openvpn.sh && ./openvpn.sh
